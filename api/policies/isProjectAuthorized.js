module.exports = async function isAuthorized(req, res, next) {
  ...
  if (!token) {
    ...
    const sharing = await Sharing.findOne({
      $and: [{ project: id }, { linkSharing: true }],
    });
  ...
  try {
    ...
    User.findOne({
      ...
      if (req.method == 'GET') {
        ...
        if (project.owner == '5c5347560d90150017d4c882') {
          return next();
        }
        if (project.owner == user._id.toString()) {
          return next();
        }

        let sharingsLenght = await Sharing.find({
          project: projectId,
        }).count();
        if (sharingsLenght > 0) {
        ...
        if (project.parent) {
          const projectParent = await Project.findById(project.parent);
          if (projectParent.owner == user._id.toString()) {
            return next();
          }

          sharingsLenght = await Sharing.find({
